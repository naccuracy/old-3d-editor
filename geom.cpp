...
bool vector2f::operator ==(vector2f &v)
{
	if((fabs(x-v.x)<=EQUAL_ERROR_GEOM)&&(fabs(y-v.y)<=EQUAL_ERROR_GEOM))return true;
	else return false;
}
bool vector2f::operator !=(vector2f &v)
{
...
float vector2f::PolarAngle()
{
	if((x==0.0)||(y==0.0))
		return -1.0;
	if(x==0.0) return ((y>0.0)? (float)90.0:(float)270.0);
	float teta=atan(y/x);
...
int vector2f::Classify(vector2f &v0, vector2f &v1)
{
	vector2f v2 = *this;
	vector2f a = v1-v0;
	vector2f b = v2-v0;
	double sa=a.x*b.y - a.y*b.x;
	if(sa>0.01)return LEFT;
	if(sa<-0.01)return RIGHT;
	if(v0==v2)return ORIGIN;
	if(v1==v2)return DESTINATION;
	if((a.x*b.x <0)||(a.y*b.y <0))return BEHIND;
...
bool vector3f::operator ==(vector3f &v)
{
	if((fabs(x-v.x)<EQUAL_ERROR_GEOM)&&(fabs(y-v.y)<EQUAL_ERROR_GEOM)&&(fabs(z-v.z)<EQUAL_ERROR_GEOM))
		return true;
	else
		return false;
}
...
vector3f vector3f::CrossProduct(vector3f &v)
{
...
int vector3f::Classify(vector3f &v0, vector3f &v1, vector3f &v2)
{
	vector3f v = *this - v0;
	float l = v.Length();
	if(l == 0.0) return ON;
...
int line2d::Intersect(line2d &l, float &t)
{
	vector2f A = a;
	vector2f B = b;
	vector2f C = l.a;
...
int line2d::Cross(line2d &l, float &t)
{
	float s=0.0;
...
int line2d::ClassifyPoint(vector2f p)
{
	switch(p.Classify(a,b))
	{
	case LEFT: return ((a.y<p.y)&&(p.y<=b.y)?CROSSING:INESSENTIAL);
	case RIGHT: return ((b.y<p.y)&&(p.y<=a.y)?CROSSING:INESSENTIAL);
	case BETWEEN:
	case ORIGIN:
	case DESTINATION:
		return TOUCHING;
...
int line3d::PointInBox(vector3f &p)
{
	int rez=0;
	if(Iscinab(a.x,b.x,p.x))rez++;
	if(Iscinab(a.y,b.y,p.y))rez++;
	if(Iscinab(a.z,b.z,p.z))rez++;
	if(rez==3)return 1;//warning
	else return 0;
}
int line3d::IntersectWithBox(line3d &bb)
{
	vector3f p;
	p=vector3f(bb.a.x,bb.a.y,bb.a.z);if(PointInBox(p))return 1;
	p=vector3f(bb.a.x,bb.a.y,bb.b.z);if(PointInBox(p))return 1;
	p=vector3f(bb.a.x,bb.b.y,bb.a.z);if(PointInBox(p))return 1;
	p=vector3f(bb.a.x,bb.b.y,bb.b.z);if(PointInBox(p))return 1;
	p=vector3f(bb.b.x,bb.a.y,bb.a.z);if(PointInBox(p))return 1;
	p=vector3f(bb.b.x,bb.a.y,bb.b.z);if(PointInBox(p))return 1;
	p=vector3f(bb.b.x,bb.b.y,bb.a.z);if(PointInBox(p))return 1;
	p=vector3f(bb.b.x,bb.b.y,bb.b.z);if(PointInBox(p))return 1;
	return 0;
}
//triangle3d
...
int triangle3d::IntersectT(triangle3d &T, vector3f_array &rez)
{
	...
	if(rez.size()==0)return NO_INTRSCT;
	//�������� ������������� �����
...
int triangle3d::HasPoint(vector3f p)
{
	...
	if(pn.CrossProduct(N()).Length()==0.0) return 1;
	else return 0;
}
double triangle3d::SGeron()
{
	double a, b, c, p;
	vector3f v;
	v=_v[0]-_v[1];a=v.Length();
	v=_v[2]-_v[1];b=v.Length();
	v=_v[0]-_v[2];c=v.Length();
	p=(a+b+c)/2;
	return sqrt(p*(p-a)*(p-b)*(p-c));
}
//iEdge
bool iEdge::operator ==(iEdge &E)
{
...
{
	return((n.end==a || n.end==b || n.end==c)&&(n.start==a || n.start==b || n.start==c));
}
...
int IsInTriangle2ds(vector2f *v, vector2f &p)
{
	...
	for(int i=0;i<3;i++)
	{
		...
		switch(e.ClassifyPoint(p))
		{
			...
			case CROSSING: rez=1-rez;break;
		}
		switch(ee.ClassifyPoint(p))
		{
			case TOUCHING: return BOUNDARY;
			case CROSSING: rez1=1-rez1;break;
		}
	}
	return rez+rez1;
}
...
{
	return(x1*y2*z3+x3*y1*z2+x2*y3*z1-x3*y2*z1-x1*y3*z2-x2*y1*z3);
}
float detv(float *x)
{
...
int TriangulatePolygon1(vector2f_array p, iEdge_array Poly, iFace_array &frez)
{
	...
	while(pn.size()<Poly.size())
	{
		for(i=1;i<Poly.size();i++)
		{
			...
			{
				if(Poly[i].start==pn[j])pn.push_back(Poly[i].end);
				if(Poly[i].end==pn[j])pn.push_back(Poly[i].start);
				j++;
				break;
			}
		}
	...
	//������������
	if(notfound)return TriangulateRing(BoundRing,frez);
	else
	{
		//����� ����������� ������
		for(i=0;i<pn.size();i++)
		...
		do
		{
			...
			for(i=0;i<pn.size();i++)
			{
				...
				if((obhod==1)&&(Z<0.001))
				{
					...
					if(zz>0.001)pn.erase(pn.begin()+i1);
					if(i1<i2)i2--;
					if(zz1>0.001)pn.erase(pn.begin()+i2);
					notfound=1;
					break;
				}
				if((obhod==0)&&(Z>-0.001))
...
int CreateTriangulationDelone1(vector2f_array &data2d, iFace_array &faces)
{
	...
	{
		Circle.push_back(GetO(data2d[faces[i].a],data2d[faces[i].b],data2d[faces[i].c]));
	}
	//���������� �����
	for(i=0;i<data2d.size();i++)
	{
		...
		for(j=0;j<faces.size();j++)
		{
			dx=O.x-Circle[j].x;
			dy=O.y-Circle[j].y;
			if(sqrt(dx*dx+dy*dy)<Circle[j].z)//����� ������ � ��������� ����������
			{
				for(k=0;k<3;k++)
		...
		for(j=0;j<SuperRing.size();j++)
		{
			for(k=j+1;k<SuperRing.size();k++)
			{
				...
				{
					SuperRing.erase(SuperRing.begin()+j);j--;
		...
		for(j=0;j<SuperRing.size();j++)
		{
			...
			Circle.push_back(GetO(data2d[SuperRing[j].start],data2d[SuperRing[j].end],data2d[i]));
		}
		}
	}
	return 1; 
...
int Mesh::Extract(FILE *f)
{
	...
	for(i=0;i<fn;i++)
	{
		fscanf_s(f,"%d %d %d",&face.a,&face.b,&face.c);
		faces.push_back(face);
	}
	
	return 1;
}
int Mesh::Load(char *file_name)
{
	int i, pn, fn;
	vector3f p;
	iFace face;
...
BooleanMesh::~BooleanMesh()
{
	...
	_B.~Mesh();
	Rez.~Mesh();
	if(in_out_A.size())in_out_A.clear();
	if(in_out_B.size())in_out_B.clear();
	if(faces_group_list_A.size())faces_group_list_A.clear();
	if(faces_group_list_B.size())faces_group_list_B.clear();
	if(intrsct_edges_A.size())intrsct_edges_A.clear();
	if(intrsct_edges_B.size())intrsct_edges_B.clear();
	if(Link_A.size())Link_A.clear();
	if(Link_B.size())Link_B.clear();
}
void Mesh::Intersect(Mesh B, Mesh &rez, iEdge_array &board)//�������� �����
{
	...
	{//main
		...
		for(j=0;j<B.faces.size();j++)
		{//j
			intrsct_points.clear();
			b=triangle3d(B.points[B.faces[j].a],B.points[B.faces[j].b],B.points[B.faces[j].c]);
			...
			if(flag==SMPL_INTRSCT)
			{//SMPL_INTRSCT
				simple=1;
				if(intrsct_points.size()>1)
				{//if
			...
			if(flag==SAME_PLANE_INTRSCT)
			{//SAME_PLANE_INTRSCT
				same=1;
				if(intrsct_points.size()>2)
		...
		{//����� ������������
			...
			{
				if(vertex_not_in_ring==0)triangulate_type=1;//������
				else triangulate_type=2;//������� � ���������� �� ������ �����
			}
			else
			{
				triangulate_type=3;//������� � ���������� �� ������ �����
			...
			case 2:
				...
				for(j=0;j<ring_faces.size();j++)
				{
					...
					for(k=0;k<all_rings.size();k++)
					{
						...
						{
							if(all_rings[k][l].HasVertex(ring_faces[j].a))flag++;
							if(all_rings[k][l].HasVertex(ring_faces[j].b))flag++;
							if(all_rings[k][l].HasVertex(ring_faces[j].c))flag++;
						}
						if(flag==6)
						{
							not_found=0;
		...
		for(j=0;j<all_faces.size();j++)
		{
			...
			if(not_found)
			{
				norm=(rez_p[all_faces[j].a]-rez_p[all_faces[j].c]).CrossProduct(rez_p[all_faces[j].b]-rez_p[all_faces[j].c]);
				norm.Normalize();
				if(norm!=normals[i])
...
int BooleanMesh::Set(Mesh A, Mesh B)
{
	...
	InOutInfo1(_A,_B,faces_group_list_A,in_out_A,intrsct_edges_A,intrsct_edges_B);
	InOutInfo1(_B,_A,faces_group_list_B,in_out_B,intrsct_edges_B,intrsct_edges_A);
	return 1;
}
int BooleanMesh::TouchBoard(iFace f, iEdge_array board)
{
	...
	{
		if(f.Edge(0)==board[i])f0=1;
		if(f.Edge(1)==board[i])f1=1;
		if(f.Edge(2)==board[i])f2=1;
	}
	return f1+f0+f2;
}

int BooleanMesh::CreateGroupList(int_array &list, Mesh A, iEdge_array intrsct, iFace_array Link)
{
	...
	for(i=0;i<A.faces.size();i++)
	{
		if(list[i]==-1)
		{
			...
			if(col==0)
			{
				if(Link[i].a!=-1)if(list[Link[i].a]==-1)stack.push_back(Link[i].a);
				if(Link[i].b!=-1)if(list[Link[i].b]==-1)stack.push_back(Link[i].b);
				if(Link[i].c!=-1)if(list[Link[i].c]==-1)stack.push_back(Link[i].c);
			}
			else
			{//��������� ������ �� ��� �� �� �� ������� �� ������� �����
				for(k=0;k<3;k++)
				{
					...
					if(not_found)
					{
						if(Link[i].Vertex(k)!=-1)if(list[Link[i].Vertex(k)]==-1)stack.push_back(Link[i].Vertex(k));
					}
				}
			}
			while(stack.size()>0)
			{
				...
				if(col==0)
				{
					if(Link[stack[0]].a!=-1)if(list[Link[stack[0]].a]==-1)stack.push_back(Link[stack[0]].a);
					if(Link[stack[0]].b!=-1)if(list[Link[stack[0]].b]==-1)stack.push_back(Link[stack[0]].b);
					if(Link[stack[0]].c!=-1)if(list[Link[stack[0]].c]==-1)stack.push_back(Link[stack[0]].c);
				}
				else
				{//��������� ������ �� ��� �� �� �� ������� �� ������� �� ������� �����
					for(k=0;k<3;k++)
					{
						...
						if(not_found)
						{
							if(Link[stack[0]].Vertex(k)!=-1)if(list[Link[stack[0]].Vertex(k)]==-1)stack.push_back(Link[stack[0]].Vertex(k));
						}
					}
				}
...
int Cube(Mesh* M, float w, float l, float h, int nw, int nl, int nh)
{
	...
	vector3f_array pv;
	iFace_array tv;
	np=(nw+1)*(nh+1)*2+(nw+1)*(nl+1)*2+(nh+1)*(nl+1)*2;
	...
	if(k!=np)return 0;
	ar=new int[np];
	for(i=0;i<np;i++)ar[i]=-1;
	//����� ������������� �����
	for(i=0;i<np;i++)
	{
		for(j=i+1;j<np;j++)
		{
			if((pv[i]==pv[j])&&(ar[j]==-1))ar[j]=i;
		}
	}
	//���������� �����
	for(i=0;i<np;i++)
	{
		if(ar[i]!=-1)
		{
			for(j=0;j<nt;j++)
			{
				if(tv[j].a==i)tv[j].a=ar[i];
				if(tv[j].b==i)tv[j].b=ar[i];
				if(tv[j].c==i)tv[j].c=ar[i];
			}
		}
	}
//�������
	...
	for(i=0;i<nt;i++)
	{
		if(i<gr[0])M->normals.push_back(vector3f(0,0,-1));
		if((i>=gr[0])&&(i<gr[1]))M->normals.push_back(vector3f(0,0,1));
		if((i>=gr[1])&&(i<gr[2]))M->normals.push_back(vector3f(0,-1,0));
		if((i>=gr[2])&&(i<gr[3]))M->normals.push_back(vector3f(0,1,0));
		if((i>=gr[3])&&(i<gr[4]))M->normals.push_back(vector3f(-1,0,0));
		if((i>=gr[4])&&(i<gr[5]))M->normals.push_back(vector3f(1,0,0));
	}
	j=0;for(i=0;i<np;i++)if(ar[i]==-1)j++;
	j=0;
	for(i=0;i<np;i++)
	{
		if(ar[i]==-1)
		{
			...
			if(i!=j)
			{
				for(k=0;k<nt;k++)
				{
					if(tv[k].a==i)tv[k].a=j;
				    if(tv[k].b==i)tv[k].b=j;
				    if(tv[k].c==i)tv[k].c=j;
				}
			}
			j++;
		}
...
int Sphere(Mesh* M, float R, int msegm, int rsegm)
{
	...
	for(i=0;i<nt;i++)
	{
		...
		b=M->faces[i].b;
		c=M->faces[i].c;
		V=vector3f(na[a].x+na[b].x+na[c].x,na[a].y+na[b].y+na[c].y,na[a].z+na[b].z+na[c].z);
		V.Normalize();
...
//vertices
	...
	m->points.push_back(vector3f(0,0,0));k++;
...
for(i=1;i<nl;i++)
{
	...
	vec[kx]=a;
	sloy[0][kx]=ar[nw][i];kx++;
}
for(i=nw;i>=0;i--)
{
	a=(m->points[0]-m->points[ar[i][nl]]);
	vec[kx]=a;
	sloy[0][kx]=ar[i][nl];kx++;
}
for(i=nl-1;i>=1;i--)
{
...
while(I<nh-1)
{
	for(j=0;j<x;j++)
	{
		...
		sloy[1][j]=k;
		a=m->points[sloy[0][j]];
		m->points.push_back(vector3f(a.x+vec[j].x,a.y+vec[j].y,a.z+vec[j].z));
		k++;
	}
	sloy[1][x]=sloy[1][0];
	//����� ����
...
int SimpleTriangulation(vector2f_array &data2d, iFace_array &faces)
{
	...
	for(i=0;i<data2d.size();i++)
	{
		...
		for(j=0;j<faces.size();j++)
		{
			if(data2d[i]==data2d[faces[j].a]||
				data2d[i]==data2d[faces[j].b]||
		...
		if(flag==1)//����� �� �������� ����� ������������
		{
			...
			for(j=0;j<numbers.size();j++)
			{
				...
				if(type[j]==BOUNDARY)
				{
					if(data2d[i].Classify(data2d[faces[n].a],data2d[faces[n].b])==BETWEEN)
					{
						faces.push_back(iFace(i,faces[n].c,faces[n].a));
						faces.push_back(iFace(i,faces[n].b,faces[n].c));
					}
					if(data2d[i].Classify(data2d[faces[n].a],data2d[faces[n].c])==BETWEEN)
					{
						faces.push_back(iFace(i,faces[n].a,faces[n].b));
						faces.push_back(iFace(i,faces[n].b,faces[n].c));
					}
					if(data2d[i].Classify(data2d[faces[n].b],data2d[faces[n].c])==BETWEEN)
					{
						faces.push_back(iFace(i,faces[n].a,faces[n].b));
						faces.push_back(iFace(i,faces[n].c,faces[n].a));
					}
...
int Prj3dTo2d(vector3f_array src, vector2f_array &dst)
{
	...
	{
		if(bBox.a.x>src[i].x)bBox.a.x=src[i].x;
		if(bBox.a.y>src[i].y)bBox.a.y=src[i].y;
		if(bBox.a.z>src[i].z)bBox.a.z=src[i].z;

		if(bBox.b.x<src[i].x)bBox.b.x=src[i].x;
		if(bBox.b.y<src[i].y)bBox.b.y=src[i].y;
		if(bBox.b.z<src[i].z)bBox.b.z=src[i].z;
	}
...
{
	vector3f nn=(n*(-1.0));
	m[0]=u.x;	m[4]=u.y;	m[8]=u.z;	m[12]=-(position.DotProduct(u));
	m[1]=v.x;	m[5]=v.y;	m[9]=v.z;	m[13]=-(position.DotProduct(v));
	m[2]=nn.x;	m[6]=nn.y;	m[10]=nn.z;	m[14]=-(position.DotProduct(nn));
	m[3]=0;		m[7]=0;		m[11]=0;	m[15]=1;
	return;
}
...
//lr=0- left =1 -right //return 0 �� ������� 1 - ������
{
	...
	Prj3dTo2d(src,dst);
	float rez;
	rez=(dst[0].x-dst[2].x)*(dst[1].y-dst[2].y)-(dst[1].x-dst[2].x)*(dst[0].y-dst[2].y);
	if(lr==0)
	{
		if(rez<0)return 0;
		else return 1;
...
int NormalizePoints(vector2f_array src, vector2f_array &dst)
{
	...
	float max;
	for(i=0;i<src.size();i++)
		if(src[min].Length()>src[i].Length())min=i;
	for(i=0;i<src.size();i++)
	{
		n=src[i]-src[min];
		dst.push_back(n);
	...
	{
		if(max<dst[i].x)max=dst[i].x;
		if(max<dst[i].y)max=dst[i].y;
	}
	for(i=0;i<dst.size();i++)
	{
...
//������ ������������ �����
	...
	for(i=0;i<edges.size();i++)
	{
		...
		for(j=0;j<all_edges.size();j++)
		{
			if(all_edges[j].HasVertex(edges[i].start)==0 && all_edges[j].HasVertex(edges[i].end)==0)
			{
		...
		for(j=0;j<pp.size();j++)
		{
			if(!all_edges[i].HasVertex(j))
			{
				...
				for(k=0;k<all_edges.size();k++)
				{
					if(k!=i)
					{
						if(all_edges[k]==iEdge(j,all_edges[i].start))notfound++;
						if(all_edges[k]==iEdge(j,all_edges[i].end))notfound++;
					}
				}
				if(notfound==2)
				{
...
int BooleanMesh::GetNeiborsInfo(Mesh m, iFace_array &neibors)
{
	...
	for(i=0;i<m.faces.size();i++)
	{
		...
		for(j=0;j<m.faces.size();j++)
		{
			if(i!=j)
			{
				if(m.faces[j].HasEdge(m.faces[i].Edge(0)))f.a=j;
				if(m.faces[j].HasEdge(m.faces[i].Edge(1)))f.b=j;
				if(m.faces[j].HasEdge(m.faces[i].Edge(2)))f.c=j;
			}
		}
		neibors.push_back(f);
	}
...
int BooleanMesh::InOutInfo1(Mesh mA, Mesh mB, int_array A_group_flags, int_array &in_out_status , iEdge_array A_board, iEdge_array B_board)
{
	int_array CurGrList;
	...
	{//main
		...
		{
			if(((mA.points[ae.start]==mB.points[B_board[j].start])&&(mA.points[ae.end]==mB.points[B_board[j].end]))||
				((mA.points[ae.start]==mB.points[B_board[j].end])&&(mA.points[ae.end]==mB.points[B_board[j].start])))
			{
				be=B_board[j];
				break;
			}
		...
		nat.Normalize();
		if(nat==nb)GrStatusList[i].end=1;//same
		else GrStatusList[i].end=4;//opposite
		if(rez>0.001)GrStatusList[i].end=2;//out
		if(rez<-0.001)GrStatusList[i].end=3;//in
	}//main
	in_out_status.clear();
...
int Schema2d::Append(FILE *f)
{
	int i;
	fprintf(f,"%d %d %f",n_view,n,size_of_n_edge);
	fprintf(f,"\nFront %d",eFront.size());
	for(i=0;i<eFront.size();i++)
	{
		fprintf(f,"\n%f %f %f %f",pFront[eFront[i].start].x,pFront[eFront[i].start].y,pFront[eFront[i].end].x,pFront[eFront[i].end].y);
	}
	fprintf(f,"\nTop %d",eTop.size());
	for(i=0;i<eTop.size();i++)
	{
		fprintf(f,"\n%f %f %f %f",pTop[eTop[i].start].x,pTop[eTop[i].start].y,pTop[eTop[i].end].x,pTop[eTop[i].end].y);
	}
	fprintf(f,"\nLeft %d",eLeft.size());
	for(i=0;i<eLeft.size();i++)
	{
		fprintf(f,"\n%f %f %f %f",pLeft[eLeft[i].start].x,pLeft[eLeft[i].start].y,pLeft[eLeft[i].end].x,pLeft[eLeft[i].end].y);
	}
	return 1;
};
int Schema2d::Save(char *fn)
{
	FILE* f=NULL;
	int i;
	fopen_s(&f,fn,"wt");
...
int Schema2d::Read(FILE *f)
{
	...
	eFront.clear();
	eLeft.clear();
	int N, i, j, k, not_found_e,not_found_s;
	char str[256];
	line2d *temp=NULL;
	vector2f_array p;
	iEdge_array e;
	iEdge te;
	//������ ���������� � ��������
	fscanf(f,"%d %d %f",&n_view,&n,&size_of_n_edge);
	...
	for(i=0;i<3;i++)
	{
		...
		for(j=0;j<N;j++)
		{
			fscanf(f,"%f %f %f %f",&(temp[j].a.x),&(temp[j].a.y),&(temp[j].b.x),&(temp[j].b.y));
		}
		//����������� ������ �����
		p.clear();
		e.clear();
...
int Schema2d::FindAndApplyO()
{
	...
	{
		if(maxx<pFront[i].x)maxx=pFront[i].x;
		if(miny>pFront[i].y)miny=pFront[i].y;
	}
	maxy=pTop[0].y;
	for(i=1;i<pTop.size();i++)
		if(maxy<pTop[i].y)maxy=pTop[i].y;
	minx=pLeft[0].x;
	for(i=1;i<pLeft.size();i++)
		if(minx>pLeft[i].x)minx=pLeft[i].x;
	...
	for(i=1;i<pFront.size();i++)
	{
		if(minxyz.x>pFront[i].x)minxyz.x=pFront[i].x;
		if(minxyz.y>pFront[i].y)minxyz.y=pFront[i].y;
		
		if(maxxyz.x<pFront[i].x)maxxyz.x=pFront[i].x;
		if(maxxyz.y<pFront[i].y)maxxyz.y=pFront[i].y;
	}
	for(i=1;i<pLeft.size();i++)
	{
		if(minxyz.z>pLeft[i].x)minxyz.z=pLeft[i].x;
		if(maxxyz.z<pLeft[i].x)maxxyz.z=pLeft[i].x;
	}
	return 1;
}
...
int Schema2d::GetBoundRect(line2d &r)
{
	...
	for(i=1;i<pFront.size();i++)
	{
		if(pf.x<pFront[i].x)pf.x=pFront[i].x;
		if(pf.y<pFront[i].y)pf.y=pFront[i].y;
	}

	pt=pTop[0];
	for(i=1;i<pTop.size();i++)
	{
		if(pf.x<pTop[i].x)pf.x=pTop[i].x;
		if(pf.y<pTop[i].y)pf.y=pTop[i].y;
	}

	pl=pLeft[0];
	for(i=1;i<pLeft.size();i++)
	{
		if(pf.x<pLeft[i].x)pf.x=pLeft[i].x;
		if(pf.y<pLeft[i].y)pf.y=pLeft[i].y;
	}
	r.a=vector2f(0,0);
...
int ExtrudeRing(Mesh* m, iEdge_array Ring, vector3f_array points, vector3f direct)
{
	...
	for(i=0;i<frez.size();i++)
	{
		nt=(points[frez[i].b]-points[frez[i].a]).CrossProduct(points[frez[i].c]-points[frez[i].a]);
		nt.Normalize();
		if(nd.DotProduct(nt)>0.0)frez[i].ChangeObhod();
	}
